...
function Prediction() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      setResult(response.data.result); // assuming backend returns { result: "Positive"/"Negative" }
    } catch (err) {
      console.error(err);
...
<div className="form-group">
  ...
  {/* PhysActivity */}
  <div className="form-group">
    <label htmlFor="PhysActivity">Physically Active (Yes=1 / No=0)</label>
    <input
      type="number"
      name="PhysActivity"
      id="PhysActivity"
  ...
  {/* HeartDiseaseorAttack */}
  <div className="form-group">
    <label htmlFor="HeartDiseaseorAttack">Heart Disease or Attack (Yes=1 / No=0)</label>
    <input
      type="number"
        ...
        <div className="prediction-result">
          {result && (
            <div className={`result-card ${result.toLowerCase()}`}>
              ...
              <p>{result === "Positive" ? "High risk of diabetes" : "Low risk of diabetes"}</p>
            </div>
          )}
